...
import { createInvoice, listInvoices, updateInvoice, deleteInvoice } from "../invoice/api-invoice"; // Import updateInvoice
...
const InvoiceManagement = () => {
  ...
  const handleAddInvoice = async (e) => {
    ...
    try {
      ...
      if (invoice.isEditing) {
        ...
        data = await updateInvoice({ invoiceId: invoice.id }, jwt.token, formattedInvoice);
      } else {
  ...
  const handleDeleteInvoice = async (id) => {
    try {
      const data = await deleteInvoice({ invoiceId: id }, jwt.token);
      if (data && data.error) {
        ...
        <CardContent>
          <form onSubmit={handleAddInvoice} className={classes.form}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Patient ID" name="patientID" value={invoice.patientID} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Appointment ID" name="appointmentID" value={invoice.appointmentID} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Amount" name="amount" type="number" value={invoice.amount} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Payment Status" name="paymentStatus" value={invoice.paymentStatus} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12}>
                <TextField
                  fullWidth
              ...
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="First Name" name="firstName" value={invoice.firstName} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Last Name" name="lastName" value={invoice.lastName} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12}>
            ...
            {invoices.map((invoice) => {
              ...
              return (
                <TableRow key={invoice._id}>
                  ...
                  <TableCell>{invoice.dateIssued.split("T")[0]}</TableCell>
