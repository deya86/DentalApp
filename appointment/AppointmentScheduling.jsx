...
const AppointmentScheduling = () => {
  ...
  const handleDeleteAppointment = async (id) => {
    try {
      const response = await removeAppointment({ appointmentId: id }, jwt);
      if (response && !response.error) {
        fetchAppointments();
      } else {
        console.error("Error deleting appointment:", response.error);
  ...
  const handleEditAppointment = (appointment) => {
    ...
    setButtonState("Update Appointment");
  };

  const handleUpdateAppointment = async (e) => {
    e.preventDefault();
    try {
      const response = await updateAppointment({ appointmentId: appointment.id }, jwt, appointment);
      if (response && !response.error) {
        fetchAppointments();
        setAppointment({
          appointmentID: "",
        ...
        setButtonState("Add Appointment");
      } else {
  ...
  const [buttonState, setButtonState] = useState("Add Appointment");
  ...
  return (
    <div>
      <Card className={classes.card}>
        ...
        <CardContent>
          <form onSubmit={buttonState === "Add Appointment" ? handleAddAppointment : handleUpdateAppointment} className={classes.form}>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Appointment ID" name="appointmentID" value={appointment.appointmentID} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Patient ID" name="patientID" value={appointment.patientID} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Dentist ID" name="dentistID" value={appointment.dentistID} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField
                  fullWidth
              ...
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Treatment Type" name="treatmentType" value={appointment.treatmentType} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12}>
                <TextField fullWidth label="Status" name="status" value={appointment.status} onChange={handleChange} required />
              </Grid>
              <Grid item xs={12}>
          ...
          <TableBody>
            {appointments.map((appointment) => (
              <TableRow key={appointment._id}>
                ...
                <TableCell>{appointment.patientID}</TableCell>
                <TableCell>{appointment.dentistID}</TableCell>
                <TableCell>{appointment.appointmentDate}12312312312</TableCell>
                <TableCell>{appointment.appointmentTime}</TableCell>
                <TableCell>{appointment.treatmentType}</TableCell>
                <TableCell>{appointment.status}</TableCell>
                <TableCell align="right">
